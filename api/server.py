...
from .insights import generate_module_insights
...
def normalize_access_scopes(scopes: list[str]) -> list[str]:
    normalized = {str(scope).strip().casefold() for scope in scopes if str(scope).strip()}
...
def status_for(module_id: str) -> dict[str, Any]:
    ...
    training_state = 'em andamento' if training_task and not training_task.done() else ('na fila' if module_id in TRAINING_QUEUED else ('pendente' if neural.get('stale') else ('atualizado' if neural.get('trained') else 'não treinado')))
    return {
        'id': module_id,
        **MODULES[module_id],
        ...
        'auto_training': os.getenv('SOFIA_AUTO_TRAIN', 'true').strip().casefold() not in {'0', 'false', 'no', 'off'},
        'ready': bool(paths),
        'pipeline': {
...
async def training_worker() -> None:
    ...
    while True:
        ...
        try:
            try:
                epochs = max(1, min(2_000, int(os.getenv('SOFIA_AUTO_TRAIN_EPOCHS', '24'))))
            except ValueError:
                epochs = 24
...
async def _train_module(module_id: str, epochs: int, learning_rate: float, trigger: str = "manual") -> dict[str, Any]:
    global TRAINING_LOCK
    if TRAINING_LOCK is None:
        TRAINING_LOCK = asyncio.Lock()
    async with TRAINING_LOCK:
        embedding_result = await asyncio.to_thread(build_module_embeddings, KNOWLEDGE_ROOT, module_id)
        result = await asyncio.to_thread(train_neural_model, KNOWLEDGE_ROOT, module_id, epochs, learning_rate, trigger)
...
def schedule_auto_training(module_id: str, reason: str = "source_update") -> bool:
    if os.getenv('SOFIA_AUTO_TRAIN', 'true').strip().casefold() in {'0', 'false', 'no', 'off'}:
        return False
...
def refresh_stale_links() -> int:
    ...
    if os.getenv('SOFIA_AUTO_REFRESH_LINKS', 'true').strip().casefold() in {'0', 'false', 'no', 'off'}:
        return 0
    try:
    ...
    for module_id in MODULES:
        ...
        for record in records:
            try:
                fetched_at = datetime.fromisoformat(str(record.get('fetched_at', '')).replace('Z', '+00:00'))
                ...
                if link_not_modified(
                    ...
                    continue
                updated = ingest_link(KNOWLEDGE_ROOT, module_id, str(record['url']), pages, 20.0, None, 1)
                local_path = KNOWLEDGE_ROOT / module_id / 'links' / str(updated['file_name'])
                ...
                logger.warning('Não foi possível atualizar link do módulo %s: %s', module_id, exc)
    return refreshed
...
async def link_refresh_loop() -> None:
    ...
    try:
        interval = max(900, int(float(os.getenv('SOFIA_LINK_REFRESH_INTERVAL_SECONDS', '3600'))))
    except ValueError:
        interval = 3600
    while True:
        ...
        try:
            ...
            if refreshed:
                ...
                logger.info('Documentos offline atualizados automaticamente: %s', refreshed)
        except asyncio.CancelledError:
            raise
        except Exception as exc:  # noqa: BLE001
...
async def expansion_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.warning('Expansão contínua falhou; a fila será retomada no próximo ciclo: %s', exc)
...
def full_context(module_id: str) -> str:
    ...
    return '\n\n--- DOCUMENTO: '.join(f'{chunk.path.name}\n{chunk.text}' for chunk in chunks)[:50000]
...
mcp = MCPServer(name='sofia-local', version='1.0.0', description='Sofia local RAG, AI and numerical tools')
...
@mcp.tool()
async def search_knowledge(module_id: str, query: str, limit: int = 6) -> dict[str, Any]:
...
@mcp.tool()
async def agent_plan(module_id: str, question: str, patient_context: bool = False) -> dict[str, Any]:
...
async def query_theme_report(module_id: str | None = None, days: int = 30, limit: int = 12) -> dict[str, Any]:
...
async def random_generate(distribution: str = 'normal', size: int = 10, seed: int | None = None) -> dict[str, Any]:
    ...
    if distribution not in {'normal', 'uniform'} or not 1 <= size <= 10000:
        raise ValueError('distribution deve ser normal/uniform e size deve estar entre 1 e 10000')
...
@mcp.tool()
async def neural_train(module_id: str, epochs: int = 120, learning_rate: float = 0.08) -> dict[str, Any]:
...
async def neural_infer(module_id: str, values: list[float]) -> dict[str, Any]:
    ...
    module_root(module_id)
    return await asyncio.to_thread(neural_model_infer, KNOWLEDGE_ROOT, module_id, values)
...
async def semantic_embed(module_id: str, force: bool = False) -> dict[str, Any]:
    ...
    enforce_mcp_admin()
    module_root(module_id)
    return await asyncio.to_thread(build_module_embeddings, KNOWLEDGE_ROOT, module_id, force)
...
@mcp.tool()
async def institutional_integration_sync(connector: str = 'tasy', resource: str = 'patients', max_pages: int = 1) -> dict[str, Any]:
...
def answer_payload(
    ...
    if not pipeline:
        pipeline = [
            {'stage': 'perceive', 'label': 'Perceber', 'status': 'complete', 'agent': 'SOFIA CORE'},
            {'stage': 'route', 'label': 'Roteiar', 'status': 'complete', 'agent': 'Planner'},
            {'stage': 'plan', 'label': 'Planejar', 'status': 'complete', 'agent': 'Planner'},
        ]
    retrieval_step = next((item for item in pipeline if item['stage'] == 'retrieve'), None)
    ...
    pipeline.append(
        {
            'stage': 'llm',
            'label': 'LLM / Provider',
            'status': 'complete' if result.provider not in {'policy', 'error'} else 'not_called',
    ...
    pipeline.append({'stage': 'output', 'label': 'Entregar', 'status': 'verified' if result.verified else 'rejected'})
    if patient_id:
        pipeline.insert(2, {'stage': 'mcp.fhir_patient_context', 'status': 'complete', 'patient_id': patient_id})
    retry_message = None
    if retry:
    ...
    learning = {
        ...
        'offline_material': {
            ...
            'sources': list(result.sources),
            'note': 'As fontes locais que fundamentaram esta resposta permanecem disponíveis offline.' if result.sources else 'Nenhuma fonte local foi recuperada.',
        },
        'external_provider_used': result.provider in {'openai', 'gemini', 'claude'},
        'offline_candidate_stored': bool(getattr(result, 'offline_material_stored', False)),
        'offline_candidate_source': getattr(result, 'offline_material_source', None),
    }
    privacy = {
        'external_context_redacted': bool(getattr(result, 'external_context_redacted', False)),
...
@mcp.tool()
async def rag_answer(module_id: str, provider: str, question: str, history: list[dict[str, str]] | None = None, patient_id: str | None = None, language: str = 'pt-BR', response_style: str = 'structured', user_code: str | None = None, retry: bool = False, retry_of: int | None = None, retry_attempt: int = 0) -> dict[str, Any]:
    ...
    trace.span('understand', 'complete', {'language': language, 'response_style': response_style})
    try:
        ...
        await asyncio.to_thread(record_search_topic, KNOWLEDGE_ROOT, module_id, question, user_code)
    ...
    provider_guard(effective_provider, patient_id=patient_id, clinical=module_id == 'medicina')
    neural_state = neural_model_status(KNOWLEDGE_ROOT, module_id)
    if (not neural_state.get('trained') or neural_state.get('stale')) and os.getenv('SOFIA_AUTO_TRAIN', 'true').strip().casefold() not in {'0', 'false', 'no', 'off'}:
    ...
    clinical_context = ''
    if patient_id:
        clinical_context = json.dumps(await asyncio.to_thread(patient_context, patient_id), ensure_ascii=False)
    allowed_external = external_clinical_allowed() if clinical_scope else external_data_allowed()
    trace.span('plan', 'complete', {'provider_mode': effective_provider, 'external_allowed': allowed_external})
    try:
...
@mcp.tool()
async def analyst_scenario(module_id: str, provider: str, question: str, baseline: float = 100.0, volatility: float = 0.1, samples: int = 5000, patient_id: str | None = None, language: str = 'pt-BR') -> dict[str, Any]:
    ...
    if not question.strip():
        raise ValueError('question não pode ser vazia')
    clinical_scope = module_id == 'medicina' or bool(patient_id)
    effective_provider = 'ollama' if provider == 'auto' and clinical_scope and not external_clinical_allowed() else provider
    provider_guard(effective_provider, patient_id=patient_id, clinical=module_id == 'medicina')
    allowed_external = external_clinical_allowed() if clinical_scope else external_data_allowed()
    return await analyze_scenario(KNOWLEDGE_ROOT, module_id, effective_provider, question, baseline, volatility, samples, patient_id, language, allowed_external)
...
class ChatRequest(BaseModel):
    module_id: str
    provider: str = Field(default='auto', pattern='^(auto|openai|gemini|claude|ollama)$')
    ...
    response_style: str = Field(default='structured', pattern='^(concise|structured|detailed)$')
    retry: bool = False
...
async def start_background_refresh() -> None:
    ...
    refresh_modules()
    initialize_expansion(KNOWLEDGE_ROOT)
    if os.getenv('SOFIA_AUTO_TRAIN_ON_STARTUP', 'true').strip().casefold() not in {'0', 'false', 'no', 'off'}:
        for module_id in MODULES:
            neural = neural_model_status(KNOWLEDGE_ROOT, module_id)
            if not neural.get('trained') or neural.get('stale'):
                schedule_auto_training(module_id)
    if os.getenv('SOFIA_AUTO_REFRESH_LINKS', 'true').strip().casefold() not in {'0', 'false', 'no', 'off'}:
    ...
    if expansion_settings()['enabled']:
        ...
        if os.getenv('SOFIA_EXPANSION_AUDIT_ON_STARTUP', 'true').strip().casefold() not in {'0', 'false', 'no', 'off'}:
            DOCUMENT_AUDIT_TASK = asyncio.create_task(audit_existing_documents_once())


@app.on_event('shutdown')
async def stop_background_refresh() -> None:
    for task in (LINK_REFRESH_TASK, EXPANSION_TASK, DOCUMENT_AUDIT_TASK, TRAINING_WORKER):
...
async def analytics_themes(
    ...
    days: int = 30,
    limit: int = 12,
    current: dict[str, Any] = Depends(require_user),  # noqa: B008
) -> dict[str, Any]:
...
async def analytics_feedback(request: FeedbackRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    """Save an optional quality signal without storing prompt or answer text."""
    try:
        updated = await asyncio.to_thread(update_feedback, KNOWLEDGE_ROOT, request.analytics_id, request.feedback, current['sub'])
    ...
    if request.feedback == 'bad':
        if current.get('sub') != ADMIN_CODE:
            ...
            if research_sources:
                assessment = {
                    **assessment,
                    'source_names': list(dict.fromkeys([*assessment.get('source_names', []), *research_sources]))[:20],
                }
    ...
    if research_result.get('stored'):
        ...
        training_scheduled = schedule_auto_training(str(assessment['module_id']), 'admin_external_research')
    elif request.feedback == 'bad' and assessment['needs_improvement'] and has_recoverable_evidence:
        ...
        training_scheduled = schedule_auto_training(str(assessment['module_id']), 'negative_feedback')
...
async def admin_expansion_status(module_id: str | None = None, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
...
async def admin_expansion_run(request: ExpansionRunRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    if request.module_id:
        module_root(request.module_id)
    result = await asyncio.to_thread(run_expansion_cycle, KNOWLEDGE_ROOT, request.module_id)
...
async def admin_expansion_pause(request: ExpansionPauseRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    ...
    return {'paused': request.paused, 'status': await asyncio.to_thread(expansion_status, KNOWLEDGE_ROOT)}


@app.post('/api/admin/expansion/domains')
async def admin_expansion_domain(request: ExpansionDomainRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    module_root(request.module_id)
    store = ExpansionStore(KNOWLEDGE_ROOT)
    await asyncio.to_thread(store.allow_domain, request.module_id, request.domain, request.mode, request.reason)
    audit_event(current['sub'], 'expansion_domain', request.module_id, request.domain)
    return {'saved': True, 'module_id': request.module_id, 'domain': request.domain, 'mode': request.mode}


@app.post('/api/admin/expansion/reprocess')
async def admin_expansion_reprocess(request: ExpansionReprocessRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    ...
    if request.file_name:
        ...
        result = await asyncio.to_thread(record_document_pipeline, KNOWLEDGE_ROOT, request.module_id, candidate)
    else:
        result = await asyncio.to_thread(audit_module_documents, KNOWLEDGE_ROOT, request.module_id)
    audit_event(current['sub'], 'expansion_reprocess', request.module_id, request.file_name or 'all')
    refresh_modules()
    return result


@app.get('/api/admin/pipeline')
async def admin_pipeline(module_id: str | None = None, limit: int = 100, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    ...
    return {'module_id': module_id, 'stages': list(PIPELINE_STAGES), 'documents': await asyncio.to_thread(pipeline_documents, KNOWLEDGE_ROOT, module_id, limit)}


@app.get('/api/admin/evaluation')
async def admin_evaluation(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
...
async def admin_semantic_evaluation(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
...
async def admin_embeddings_status(module_id: str | None = None, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
...
async def admin_readiness(module_id: str | None = None, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
...
async def admin_embeddings_build(module_id: str | None = None, force: bool = False, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    """Build local embeddings sequentially, optionally for one module."""
    require_statistics_admin(current)
    if module_id:
        module_root(module_id)
        result = await asyncio.to_thread(build_module_embeddings, KNOWLEDGE_ROOT, module_id, force)
    else:
        result = await asyncio.to_thread(build_all_embeddings, KNOWLEDGE_ROOT, sorted(MODULES), force)
    audit_event(current['sub'], 'embedding_build', module_id or 'all-modules')
    return result


@app.get('/api/admin/observability')
async def admin_observability(module_id: str | None = None, limit: int = 100, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    return await asyncio.to_thread(observability_snapshot, KNOWLEDGE_ROOT, module_id, limit)


@app.get('/api/admin/insights')
async def admin_insights(module_id: str | None = None, limit: int = 100, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    return await asyncio.to_thread(insights_snapshot, KNOWLEDGE_ROOT, module_id, limit)


@app.post('/api/admin/insights/run')
async def admin_insights_run(module_id: str, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_statistics_admin(current)
    module_root(module_id)
    result = await asyncio.to_thread(generate_module_insights, KNOWLEDGE_ROOT, module_id)
...
@app.get('/api/auth/me')
async def me(user: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    return {'user': {key: value for key, value in user.items() if key not in {'sub', 'sid'}}}


@app.post('/api/auth/logout')
async def logout(http_request: Request, user: dict[str, Any] = Depends(require_user)) -> dict[str, bool]:  # noqa: B008
...
async def token_rotate(http_request: Request, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    header = http_request.headers.get('Authorization', '')
...
async def request_access(request: AccessRequestCreate) -> dict[str, Any]:
    ...
    try:
        result = create_access_request(request.name, request.email, request.requested_module, scopes)
    except sqlite3.IntegrityError as exc:
        raise HTTPException(409, 'O código de usuário ou solicitação já existe') from exc
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return {'created': True, **result, 'message': 'Solicitação enviada. Aguarde a aprovação da AG000001.'}


@app.get('/api/auth/requests')
async def access_requests(status: str = 'pending', current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
...
async def access_decision(request_id: int, request: AccessDecisionRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    scopes = normalize_access_scopes(request.scopes) if request.approve and request.scopes else []
    try:
        result = decide_access_request(request_id, request.approve, scopes, ADMIN_CODE, request.note)
    except PermissionError as exc:
        raise HTTPException(403, str(exc)) from exc
    except sqlite3.IntegrityError as exc:
        ...
        raise HTTPException(400, str(exc)) from exc
    audit_event(current['sub'], 'access_decision', str(request_id), result.get('status', ''))
    return result


@app.post('/api/auth/users')
async def users_endpoint(request: UserCreateRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    scopes = normalize_access_scopes(request.scopes)
    primary_module = request.primary_module or (scopes[0] if scopes and scopes[0] != 'CORE' else None)
    if primary_module and primary_module.casefold() not in MODULES:
        raise HTTPException(400, 'Módulo principal inválido')
    try:
        user = create_user(request.email, request.name, request.password, scopes=scopes, primary_module=primary_module)
    except sqlite3.IntegrityError as exc:
...
async def users_list(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    return {'users': list_users(), 'inactivity': get_inactivity_policy(), 'session_rotation_seconds': 600}


@app.post('/api/auth/users/inactivity-policy')
async def users_inactivity_policy(request: InactivityPolicyRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    try:
...
async def user_password_reset(user_code: str, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    try:
    ...
    return {'created': True, **result, 'message': 'Token de recuperação criado. Ele expira em 10 minutos e só pode ser usado uma vez.'}


@app.post('/api/auth/users/{user_code}/status')
async def user_status(user_code: str, request: UserStatusRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    try:
        user = set_user_active(user_code, request.active, request.reason)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    audit_event(current['sub'], 'user_status', user_code, 'active' if request.active else 'blocked')
...
async def account_activation(request: AccountActivationRequest) -> dict[str, Any]:
    try:
        artifact = activate_account(request.user_code, request.activation_token, request.new_password)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return {**artifact, 'message': 'Senha criada. Adicione o QR ao seu aplicativo autenticador e valide o código para concluir.'}


@app.post('/api/auth/activation/2fa')
async def activation_two_factor(request: ActivationTwoFactorRequest) -> dict[str, Any]:
    try:
        enable_activation_two_factor(request.user_code, request.activation_token, request.code)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return {'enabled': True, 'message': 'Conta ativada. Agora entre com o e-mail ou a matrícula gerada.'}


@app.post('/api/auth/password/reset')
async def password_reset(request: PasswordResetRequest) -> dict[str, Any]:
    try:
        result = reset_password_with_token(request.user_code, request.reset_token, request.new_password)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return {'reset': True, **result, 'message': 'Senha redefinida. Entre com a matrícula ou e-mail cadastrado.'}


@app.post('/api/auth/password')
async def password_change(request: PasswordChangeRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, bool]:  # noqa: B008
    try:
...
async def two_factor_setup(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    return setup_two_factor(ADMIN_CODE)


@app.post('/api/auth/2fa/enable')
async def two_factor_enable(request: TwoFactorCodeRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, bool]:  # noqa: B008
    require_admin(current)
    try:
...
@app.get('/api/modules')
async def modules_endpoint(current: dict[str, Any] = Depends(require_user)) -> list[dict[str, Any]]:  # noqa: B008
    refresh_modules()
    return [status_for(module_id) for module_id in MODULES if has_module_access(current, module_id)]


@app.get('/api/domain-contracts')
async def domain_contracts_endpoint(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    refresh_modules()
    visible = [module_id for module_id in MODULES if has_module_access(current, module_id)]
    return {'domains': manifests(visible), 'contract_version': '1.0'}


@app.get('/api/tools')
async def tools_endpoint(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    del current
    return {'tools': tool_contracts(), 'allowlist_only': True, 'os_access': False, 'arbitrary_network_access': False}


@app.get('/api/capabilities')
async def capabilities(_: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    privacy = privacy_status()
    openai_key = bool(os.getenv('OPENAI_API_KEY'))
    openai_store = os.getenv('OPENAI_STORE_RESPONSES', 'false').strip().casefold() in {'1', 'true', 'yes', 'on', 'sim'}
    ...
    return {
        ...
        'models': {'ollama': os.getenv('OLLAMA_MODEL', 'qwen3.5:4b'), 'openai': os.getenv('OPENAI_MODEL', 'gpt-5.5'), 'gemini': os.getenv('GEMINI_MODEL', 'gemini-3.6-flash'), 'claude': os.getenv('ANTHROPIC_MODEL', 'claude-sonnet-4-6')},
        'privacy': privacy,
        'integrations': integration_status(),
        'training': {
            'sequential': True,
            'queued_modules': sorted(TRAINING_QUEUED),
            'active_modules': sorted(module_id for module_id, task in AUTO_TRAINING.items() if not task.done()),
        },
        'expansion': {
...
@app.get('/api/privacy')
async def privacy(_: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    return privacy_status()


@app.get('/api/integrations')
async def integrations(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    return await asyncio.to_thread(integration_status)


@app.post('/api/integrations/{connector}/sync')
async def integrations_sync(connector: str, request: IntegrationSyncRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_admin(current)
    try:
        result = await asyncio.to_thread(sync_integration, connector, request.resource, request.max_pages)
...
async def upload(module_id: str, file: UploadFile = File(...), current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    module_root(module_id)
    ...
    destination_root = module_root(module_id) / ('imagens' if suffix in IMAGE_EXTENSIONS else 'textos')
    ...
    for existing in files_for(KNOWLEDGE_ROOT, module_id):
        ...
        if hashlib.sha256(existing.read_bytes()).digest() == digest:
            raise HTTPException(409, f'Arquivo duplicado: conteúdo já existe em {existing.name}')
...
async def add_link(module_id: str, request: LinkCreateRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    module_root(module_id)
    require_module_access(current, module_id)
    max_pages = max(request.max_pages, 10) if request.dense else request.max_pages
    try:
        record = await asyncio.to_thread(ingest_link, KNOWLEDGE_ROOT, module_id, request.url, max_pages, 20.0, None, request.max_depth)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    except RuntimeError as exc:
...
async def module_links(module_id: str, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    module_root(module_id)
    require_module_access(current, module_id)
    repository = LinkRepository(KNOWLEDGE_ROOT)
    try:
...
async def fhir_metadata(current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_module_access(current, 'medicina')
    return capability_statement()


@app.get('/fhir/{resource_type}')
async def fhir_search(resource_type: str, patient: str | None = None, code: str | None = None, _count: int = 50, _: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_module_access(_, 'medicina')
    try:
        return await asyncio.to_thread(search_resources, resource_type, patient, code, _count)
    except ValueError as exc:
        raise HTTPException(404, str(exc)) from exc


@app.get('/fhir/{resource_type}/{resource_id}')
async def fhir_read(resource_type: str, resource_id: str, _: dict[str, Any] = Depends(require_user)) -> JSONResponse:  # noqa: B008
    require_module_access(_, 'medicina')
    try:
...
async def fhir_create(resource_type: str, resource: dict[str, Any], current: dict[str, Any] = Depends(require_user)) -> JSONResponse:  # noqa: B008
    require_module_access(current, 'medicina')
    if current.get('role') not in {'admin', 'clinician'}:
        raise HTTPException(403, 'Somente administradores ou profissionais autorizados podem gravar recursos FHIR')
    try:
    ...
    return JSONResponse(stored, status_code=201, media_type='application/fhir+json', headers={'Location': f'/fhir/{resource_type}/{stored["id"]}'})


@app.put('/fhir/{resource_type}/{resource_id}')
async def fhir_update(resource_type: str, resource_id: str, resource: dict[str, Any], current: dict[str, Any] = Depends(require_user)) -> JSONResponse:  # noqa: B008
    require_module_access(current, 'medicina')
    if current.get('role') not in {'admin', 'clinician'}:
        raise HTTPException(403, 'Somente administradores ou profissionais autorizados podem gravar recursos FHIR')
...
async def fhir_patient_context(patient_id: str, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
    require_module_access(current, 'medicina')
    return await asyncio.to_thread(patient_context, patient_id)


@app.post('/api/chat')
async def chat(request: ChatRequest, current: dict[str, Any] = Depends(require_user)) -> dict[str, Any]:  # noqa: B008
...
async def run_tool(tool_name: str, request: ToolRequest, current: dict[str, Any] = Depends(require_user)) -> Any:  # noqa: B008
    ...
    if tool_name in {'institutional_integration_status', 'institutional_integration_sync'}:
        require_admin(current)
    if tool_name in {'semantic_embed', 'readiness_check'}:
        require_admin(current)
    if tool_name == 'query_theme_report':
        require_statistics_admin(current)
    audit_event(current['sub'], contract.audit_event, tool_name, contract.required_capability)
    try:
...
if __name__ == '__main__':
    if os.getenv('SOFIA_TRANSPORT') == 'stdio':
        ...
        import uvicorn
        uvicorn.run('api.server:app', host=os.getenv('SOFIA_BIND_HOST', '0.0.0.0'), port=8787, reload=False)
