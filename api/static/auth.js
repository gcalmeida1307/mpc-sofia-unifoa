const sofia=(()=>{
  ...
  async function initAuth(){
    ...
    document.querySelectorAll('[data-admin]').forEach(element=>element.hidden=user.role!=='admin');const nav=document.querySelector('.topbar nav');if(nav){buildNavigation(nav,user);let toggle=document.querySelector('.nav-toggle');if(!toggle){toggle=document.createElement('button');toggle.type='button';toggle.className='nav-toggle';toggle.setAttribute('aria-label','Abrir ou recolher navegação');toggle.innerHTML='<span></span><span></span><span></span>';document.querySelector('.topbar .brand').after(toggle)}toggle.onclick=()=>{if(matchMedia('(max-width:900px)').matches)document.body.classList.toggle('nav-open');else{document.body.classList.toggle('nav-collapsed');localStorage.setItem('sofia-nav-collapsed',document.body.classList.contains('nav-collapsed')?'1':'0')}};if(localStorage.getItem('sofia-nav-collapsed')==='1')document.body.classList.add('nav-collapsed')}
