const sofia=(()=>{
  ...
  async function initAuth(){
    ...
    document.querySelectorAll('[data-admin]').forEach(element=>element.hidden=user.role!=='admin');const nav=document.querySelector('.topbar nav');if(nav){buildNavigation(nav,user);document.body.classList.remove('nav-collapsed');localStorage.removeItem('sofia-nav-collapsed');let toggle=document.querySelector('.nav-toggle');if(!toggle){toggle=document.createElement('button');toggle.type='button';toggle.className='nav-toggle';toggle.setAttribute('aria-label','Abrir navegação');toggle.innerHTML='<span></span><span></span><span></span>';document.querySelector('.topbar .brand').after(toggle)}toggle.onclick=()=>document.body.classList.toggle('nav-open')}
