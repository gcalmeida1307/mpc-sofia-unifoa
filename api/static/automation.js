...
function renderCatalog(){byId('connector-catalog').innerHTML=catalog.map(item=>`<button class="connector-card ${item.status}" data-type="${item.type}"><span>${escapeHtml(item.label)}</span><small>${escapeHtml(item.category)} · ${item.status==='active'?'pronto':'requer configuração'}</small></button>`).join('');document.querySelectorAll('.connector-card').forEach(button=>{
...
function loadSqlTemplate(){
  const sequence=['trigger','grafana','prometheus','loki','sql','correlate','report'];
  ...
  graphId=null;byId('graph-name').value='Investigar lentidão SQL';byId('graph-description').value='Correlaciona métricas, logs, banco e eventos para explicar degradação SQL.';renderGraph();
}
...
(async()=>{await sofia.initAuth();catalog=(await sofia.api('/workflows/automation/connectors')).connectors;renderCatalog();await Promise.all([loadGraphs(),loadAdmin()]);const canvas=byId('graph-canvas');canvas.ondragover=event=>{event.preventDefault();event.dataTransfer.dropEffect='copy';canvas.classList.add('drop-active')};canvas.ondragleave=()=>canvas.classList.remove('drop-active');canvas.ondrop=event=>{event.preventDefault();canvas.classList.remove('drop-active');const type=event.dataTransfer.getData('application/x-sofia-connector');const item=connector(type);if(!type||!catalog.some(x=>x.type===type))return;const rect=canvas.getBoundingClientRect();const node={id:crypto.randomUUID(),type:item.type,label:item.label,x:Math.max(0,event.clientX-rect.left+canvas.scrollLeft-85),y:Math.max(0,event.clientY-rect.top+canvas.scrollTop-40)};nodes.push(node);renderGraph()};byId('sql-template').onclick=loadSqlTemplate;byId('connect-selected').onclick=connectSelected;byId('clear-graph').onclick=()=>{nodes=[];edges=[];selected=[];graphId=null;renderGraph()};byId('save-graph').onclick=async()=>{await saveGraph();alert('Fluxo salvo.')};byId('simulate-graph').onclick=simulate;renderGraph()})().catch(e=>alert(e.message));
