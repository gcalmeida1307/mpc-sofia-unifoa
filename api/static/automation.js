...
function buildTemplate(key){
  ...
  nodes=template.types.filter(type=>catalog.some(item=>item.type===type)).map((type,index)=>({id:createNodeId(),type,label:connector(type).label,x:45+(index%3)*220,y:45+Math.floor(index/3)*130,config:key==='overnight'&&type==='zabbix'?{operation:'history',period:'overnight'}:{}}));
