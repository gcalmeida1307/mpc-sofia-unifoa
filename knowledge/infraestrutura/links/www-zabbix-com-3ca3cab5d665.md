...
Fonte: https://www.zabbix.com/documentation/7.4/pt/manual/config/hosts/host
Capturado em: 2026-09-01T15:38:13.965181+00:00
Páginas no domínio: 10

## 2 Configurando um host
URL: https://www.zabbix.com/documentation/7.4/pt/manual/config/hosts/host
...
2 Configurando um host
Visão geral
Para configurar um host no frontend do Zabbix, faça o seguinte:
Acesse:
...
Clique em
Criar host
no canto superior direito da tela (ou no nome do host para editar um host existente)
Insira os parâmetros do host no formulário
Você também pode usar o botão
Clonar
no formulário de configuração de um host existente para criar um novo host. Esse host terá todas as propriedades do host existente, incluindo templates vinculados, entidades (items, triggers, etc.) desses templates, bem como as entidades diretamente associadas ao host existente.
Observe que, quando um host é clonado, ele manterá todas as entidades do template exatamente como estão originalmente no template. Quaisquer alterações feitas nessas entidades no nível do host existente (como intervalo do item alterado, expressão regular modificada ou protótipos adicionados à regra de descoberta em baixo nível) não serão clonadas para o novo host; em vez disso, elas permanecerão como no template.
Como alternativa, você pode usar o
Host Wizard
para configurar um host por meio de uma interface guiada, passo a passo.
Configuração
A aba
Host
contém atributos gerais do host:
Todos os campos obrigatórios são marcados com um asterisco vermelho.
Parameter
Description
Host name
Informe um nome de host exclusivo. São permitidos caracteres alfanuméricos, espaços, pontos, hífens e sublinhados. No entanto, espaços no início e no fim não são permitidos.
Note:
Com o Zabbix agent em execução no host que você está configurando, o parâmetro
Hostname
do
arquivo de configuração
do agent deve ter o mesmo valor do nome de host informado aqui. O nome no parâmetro é necessário no processamento de
active checks
.
Visible name
Informe um nome visível exclusivo para o host. Se você definir esse nome, ele será o exibido em listas, mapas etc., em vez do nome técnico do host. Este atributo tem suporte a UTF-8.
Templates
Associe
templates
ao host. Todas as entidades (items, triggers,
etc.
) serão herdadas do template.
Para associar um novo template, comece a digitar o nome do template no campo de texto. Uma lista de templates correspondentes será exibida; role para baixo para selecionar. Como alternativa, você pode clicar em
Select
ao lado do campo e selecionar templates na lista em uma janela pop-up. Todos os templates selecionados serão associados ao host quando o formulário de configuração do host for salvo ou atualizado.
Para desassociar um template, use uma das duas opções no bloco
Linked templates
:
Unlink
- desassocia o template, mas preserva suas entidades (items, triggers,
etc.
);
Unlink and clear
- desassocia o template e remove todas as suas entidades (items, triggers,
etc.
).
Os nomes dos templates listados são links clicáveis que levam ao formulário de configuração do template.
Host groups
Selecione os
host groups
aos quais o host pertence. Um host deve pertencer a pelo menos um host group. Um novo grupo pode ser criado e associado ao host adicionando um nome de grupo inexistente.
Interfaces
Vários tipos de interface de host são suportados para um host:
Agent
,
SNMP
,
...
IPMI
.
Nenhuma interface é definida por padrão. Para adicionar uma nova interface, clique em
Add
no bloco
Interfaces
, selecione o tipo de interface e informe os dados de
IP/DNS
,
Connect to
e
...
.
Note:
Interfaces usadas em qualquer item não podem ser removidas e o link
Remove
fica esmaecido para elas.
O "IP" ou "DNS" de uma interface SNMP também é usado para
SNMP traps
.
Durante a correspondência, apenas o "IP" ou "DNS" selecionado na interface do host é usado.
Consulte
Configuring SNMP monitoring
para obter detalhes adicionais sobre a configuração de uma interface SNMP (v1, v2 e v3).
IP address
Endereço IP do host (opcional).
DNS name
Nome DNS do host (opcional).
Connect to
Ao clicar no respectivo botão, você informa ao Zabbix server o que usar para recuperar dados dos agents:
IP
- Conectar ao endereço IP do host (recomendado)
DNS
- Conectar ao nome DNS do host
Port
Número da porta TCP/UDP. Os valores padrão são: 10050 para Zabbix agent, 161 para SNMP agent, 12345 para JMX e 623 para IPMI.
Default
Marque o botão de opção para definir a interface padrão.
Description
Informe a descrição do host.
Monitored by
Selecione se o host é monitorado por:
Server
- o host é monitorado pelo Zabbix server;
Proxy
- o host é monitorado por um proxy independente;
Proxy group
- o host é monitorado por um grupo de proxy.
Proxy
O nome do proxy atribuído é exibido (somente se o Zabbix server tiver atribuído um a partir do grupo de proxy selecionado).
Este campo é exibido somente se o host for monitorado por um grupo de proxy.
Enabled
Quando a caixa de seleção está marcada, o host está habilitado - pronto para monitoramento.
Quando a caixa de seleção está desmarcada, o host está desabilitado - não monitorado:
Para solicitações de dados passivos iniciadas pelo Zabbix server/proxy (por exemplo,
Zabbix agent
,
SNMP agent
,
simple checks
), o monitoramento é desabilitado após a sincronização da configuração. Triggers e ações vinculadas ao host também são desabilitadas somente após o recarregamento da configuração.
Para
active checks
do Zabbix agent, o monitoramento é interrompido dentro do intervalo de tempo (aprox. 5 segundos) em que o Zabbix agent recebe a informação de que o host foi desabilitado. Durante esse breve intervalo, o host continuará coletando dados localmente para os active checks e tentando enviá-los ao server/proxy; no entanto, como o host está marcado como
Disabled
, o server/proxy rejeitará os dados.
Quando você desabilita o host, seus items são removidos imediatamente do cache de histórico (exceto pelos últimos valores, que são mantidos para logs).
A aba
IPMI
contém atributos de gerenciamento IPMI.
Parameter
Description
...
Username
Nome de usuário para autenticação. Macros de usuário podem ser usadas.
Password
Senha para autenticação. Macros de usuário podem ser usadas.
A aba
Tags
permite definir
tags
no nível do host. Todos os problemas deste host serão marcados com os valores informados aqui.
Macros de usuário, macros {INVENTORY.*}, {HOST.HOST}, {HOST.NAME}, {HOST.CONN}, {HOST.DNS}, {HOST.IP}, {HOST.PORT} e {HOST.ID} são suportadas em tags.
A aba
Macros
permite definir
macros de usuário
no nível do host como pares nome-valor. Observe que os valores das macros podem ser mantidos como texto simples, texto secreto ou segredo do Vault. Também há suporte para adicionar uma descrição.
Você também pode visualizar aqui macros de usuário globais e de nível de template se selecionar a opção
Inherited and host macros
. É aí que todas as macros de usuário definidas para o host são exibidas com o valor para o qual são resolvidas, bem como sua origem.
Para conveniência, são fornecidos links para os respectivos templates e para a configuração de macros globais. Também é possível editar uma macro de template/global no nível do host, criando efetivamente uma cópia da macro no host.
A aba
Inventory
permite inserir manualmente informações de
inventory
para o host. Você também pode selecionar para habilitar o preenchimento de inventory
Automatic
, ou desabilitar o preenchimento de inventory para este host.
Se o inventory estiver habilitado (manual ou automático), um ponto verde é exibido com o nome da aba.
Criptografia
A aba
Criptografia
permite que você exija conexões
criptografadas
com o host.
Parameter
Description
Connections to host
Como o server ou proxy do Zabbix se conecta ao agent do Zabbix em um host: sem criptografia (padrão), usando PSK (chave pré-compartilhada) ou certificado.
Connections from host
Selecione que tipo de conexões são permitidas a partir do host (ou seja, do agent do Zabbix e do Zabbix sender). Vários tipos de conexão podem ser selecionados ao mesmo tempo (útil para testes e para alternar para outro tipo de conexão). O padrão é "No encryption".
Issuer
Emissor permitido do certificado. O certificado é primeiro validado com a CA (autoridade certificadora). Se ele for válido, assinado pela CA, então o campo
Issuer
pode ser usado para restringir ainda mais a CA permitida. Este campo deve ser usado se sua instalação do Zabbix utilizar certificados de várias CAs. Se este campo estiver vazio, qualquer CA será aceita.
Subject
Subject permitido do certificado. O certificado é primeiro validado com a CA. Se ele for válido, assinado pela CA, então o campo
Subject
pode ser usado para permitir apenas um valor da string
Subject
. Se este campo estiver vazio, qualquer certificado válido assinado pela CA configurada será aceito.
PSK identity
String de identidade da chave pré-compartilhada.
...
PSK
Chave pré-compartilhada (string hexadecimal). Comprimento máximo: 512 dígitos hexadecimais (PSK de 256 bytes) se o Zabbix usar a biblioteca GnuTLS ou OpenSSL, 64 dígitos hexadecimais (PSK de 32 bytes) se o Zabbix usar a biblioteca mbed TLS (PolarSSL). Exemplo: 1f87b595725ac58dd977beef14b97461a7c1045b9a1c963065002c5473194952
Mapeamento de valores
A aba
Mapeamento de valores
permite configurar uma representação amigável dos dados do item em
...
## Manual do usuário
URL: https://www.zabbix.com/documentation/7.4/pt/manual
...
## 20 API
URL: https://www.zabbix.com/documentation/7.4/pt/manual/api
...
## 7 Configuração
URL: https://www.zabbix.com/documentation/7.4/pt/manual/config
...
## 23 Guias de referência rápida
URL: https://www.zabbix.com/documentation/7.4/pt/manual/guides
...
## 22 Apêndices
URL: https://www.zabbix.com/documentation/7.4/pt/manual/appendix
...
## 3 processos do Zabbix
URL: https://www.zabbix.com/documentation/7.4/pt/manual/concepts
...
## 6 Zabbix appliance
URL: https://www.zabbix.com/documentation/7.4/pt/manual/appliance
...
## 15 Descoberta
URL: https://www.zabbix.com/documentation/7.4/pt/manual/discovery
...
## 17 Criptografia
URL: https://www.zabbix.com/documentation/7.4/pt/manual/encryption
