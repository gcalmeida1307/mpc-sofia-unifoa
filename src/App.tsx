...
function PipelineExplorer({
  ...
  const stageLabel = (stage: string) =>
    stage === "READY" ? "Pronto" : stage.replaceAll("_", " ")
  return (
    <div className="page-body explorer-page">
      <div className="page-heading-row">
        ...
        <div className="explorer-toolbar">
          ...
          <div className="explorer-actions">
            <button className="neural-run" onClick={() => void load()} disabled={busy}>
              {busy ? "Atualizando..." : "Atualizar"}
            </button>
            <button className="neural-run secondary" onClick={() => void evaluate()} disabled={evaluating}>
      ...
      <div className="explorer-health-grid">
        ...
        <article className="metric-card">
          ...
          <span>{embeddingRecord?.status ?? "pending"} · {embeddings?.model ?? "embedding local"}</span>
        </article>
      </div>
      {readiness && (
        <section className="explorer-panel readiness-panel">
          <div className="section-heading readiness-heading">
            <div>
              ...
              <p>Dez níveis verificáveis para saber exatamente o que está pronto, parcial ou bloqueado.</p>
            </div>
            <div className={`readiness-banner ${readiness.scale_ready ? "ready" : "attention"}`}>
              <strong>{readiness.ready_levels}/10</strong>
              <span>{readiness.scale_ready ? "Escala aprovada" : "Ainda não aprovado para escala"}</span>
            </div>
          </div>
            ...
            {readiness.levels.map((level) => (
              <article className={`readiness-row readiness-${level.status}`} key={level.id}>
                <div className="readiness-level">
                  <span className="readiness-number">{String(level.id).padStart(2, "0")}</span>
                  <div>
                ...
                <div className="readiness-content">
                  <p>{level.details}</p>
                  <ul>
                    {level.evidence.map((item, index) => <li key={`${level.id}-${index}`}>{item}</li>)}
                  </ul>
                  {level.action && <small className="readiness-action">Próximo passo: {level.action}</small>}
                </div>
                <div className="readiness-score" aria-label={`Score ${level.score}%`}>
          ...
          <small className="readiness-footnote">
            Avaliado em {new Date(readiness.evaluated_at).toLocaleString("pt-BR")}. Números ausentes não são estimados.
          </small>
        </section>
      )}
      <section className="explorer-panel">
        <div className="section-heading">
          <div>
            ...
            <p>Falhas permanecem identificadas e podem ser reprocessadas; metadados incompletos não viram “pronto”.</p>
          </div>
          <span className="status-pill active">somente administrador</span>
        </div>
        {documents.length === 0 ? (
          <div className="empty-state">Nenhum documento registrado neste módulo.</div>
        ) : (
                ...
                {documents.map((document) => {
                  ...
                  return (
                    <tr key={String(document.id)}>
                      <td>
                        <strong>{String(document.file_name ?? "Documento")}</strong>
                        <small>{String(document.mime_type ?? "")} · {String(document.source_origin ?? "origem não informada")} · v{String(document.version_number ?? "—")}</small>
                        <small>{String(document.sensitivity ?? "sensibilidade não informada")}</small>
                      </td>
                      <td><span className={`pipeline-status ${status.toLowerCase()}`}>{stageLabel(status)}</span></td>
                      <td>{document.extraction_quality == null ? "—" : `${Math.round(Number(document.extraction_quality) * 100)}%`}</td>
                      <td>{String(document.chunk_count ?? 0)}</td>
                      <td>
                        <div className="stage-chips">
                          {events.map((event) => (
                            <span key={`${String(event.stage)}-${String(event.started_at)}`} title={String(event.error_message ?? "")} className={String(event.status).toLowerCase()}>
      ...
      <section className="explorer-panel">
        <div className="section-heading">
          <div>
            ...
            <p>Latência, modelo, evidência, confiança, tokens e custo quando retornados; sem armazenar o conteúdo da pergunta.</p>
          </div>
        </div>
        <div className="trace-list">
          {(observability?.traces ?? []).slice(0, 6).map((trace) => {
            ...
            try {
              metrics = JSON.parse(String(trace.metrics_json ?? "{}")) as Record<string, unknown>
            } catch {
              metrics = {}
            }
            ...
            return (
              <div className="trace-row" key={String(trace.trace_id)}>
                ...
                <div className="trace-primary">
                  ...
                  <small>{String(trace.model ?? "modelo não registrado")} · {String(trace.complexity ?? "L1")}</small>
                </div>
                <div className="trace-metric">
                  <strong>{trace.latency_ms == null ? "—" : `${Math.round(Number(trace.latency_ms))} ms`}</strong>
                  <small>latência</small>
                </div>
                <div className="trace-metric">
                  <strong>{trace.confidence == null ? "—" : `${Math.round(Number(trace.confidence) * 100)}%`}</strong>
                ...
                <div className="trace-metric trace-optional">
                  ...
                  <small>tokens · custo {cost == null ? "—" : String(cost)}</small>
                </div>
              </div>
            )
          })}
          {!observability?.traces?.length && <div className="empty-state">Ainda não há traces para este módulo.</div>}
        </div>
      </section>
      {evaluation && (
        <section className="explorer-panel">
          ...
          <div className="evaluation-grid">
            {evaluation.modules.map((item) => (
              <div className="evaluation-row" key={item.module}>
                ...
                <span>{item.status === "ready" ? "pronto" : "precisa de dados"}</span>
                <b>{item.score}%</b>
              </div>
            ))}
          </div>
          {evaluation.semantic_evaluation && (
            <div className="evaluation-summary">
              <strong>Avaliação semântica de evidências: {evaluation.semantic_evaluation.global_score}%</strong>
              <span>{evaluation.semantic_evaluation.case_count} casos revisáveis · cobertura de termos e aderência às fontes</span>
            </div>
          )}
        </section>
      )}
        ...
        {stages.map(([id, title, fallbackDetail, icon], index) => {
          ...
          return (
            <span
              ...
              key={id}
              title={current?.detail ?? fallbackDetail}
            >
              <i>{icon}</i>
              <b>{title}</b>
              <small>
                {current ? statusLabel[status] ?? status : fallbackDetail}
              </small>
            </span>
          )
        }).flatMap((node, index) =>
          index < stages.length - 1
            ? [node, <em key={`arrow-${index}`}>→</em>]
            : [node],
        )}
      </div>
      <div className="pipeline-signal" aria-live="polite">
        <span className="pipeline-signal-dot" />
        <b>{trace?.length ? "Pipeline concluído" : "Pipeline pronto"}</b>
        <span>
          {sourceCount
            ? `${sourceCount} fonte(s) local(is) consultada(s)`
            : "Aguardando uma pergunta para consultar o RAG"}
        </span>
        <span>IA: {provider ?? "aguardando"}</span>
        <span>{verified ? "✓ resposta verificada" : "Rede neural e Monte Carlo disponíveis em Análise"}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="chat-page">
      <div className="rag-bar">
        <span /> RAG {mod.name} — {mod.docs} documentos{" "}
        <div className="chat-options">
          ...
          <b>
            {provider === "auto" ? "AUTO · fallback seguro" : provider.toUpperCase()} · RAG local
          </b>
        </div>
      </div>
      {mod.id === "medicina" && (
        <div className="clinical-context">
          ...
          <input
            ...
            disabled={busy}
          />
          <small>contexto local · provider externo bloqueado por padrão</small>
        </div>
      )}
      <Pipeline
      ...
      <div className="chat-messages" role="log" aria-live="polite">
        <div className="ai-row">
          <div className="chat-avatar" style={{ background: mod.color }}>
            S
          </div>
          <div>
        ...
        {chat.map((item, index) =>
          item.role === "user" ? (
            <div className="user-row" key={index}>
              <div className="bubble">
                {item.attachment && (
                  <span className="attachment-chip">📎 {item.attachment}</span>
                )}
            ...
            <div className="ai-row" key={index}>
              ...
              <div>
                ...
                <div className="message-meta">
                  {item.agent_trace && item.agent_trace.length > 0 && (
                    <small className="agent-trace">
                      Fluxo:{" "}
                      {item.agent_trace
                        ...
                        .map((stage) => {
                          ...
                          return `${stage.stage} · ${labels[stage.status] ?? stage.status}`
                        })
                        .join(" → ")}
                    </small>
                  )}
                  <small className="message-provider">
                    {item.provider ?? provider}
                  </small>
                  {item.analytics_id && (
                    <span className="message-feedback" aria-label="Avaliar resposta">
                      <button
                        type="button"
                        className={item.feedback === "good" ? "active" : ""}
                        onClick={() =>
                          sendFeedback(item.analytics_id!, "good")
                        }
                      >
                        ✓ Útil
                      </button>
                      ...
                      {item.feedback === "bad" && item.retry_question && (
                        (item.retry_attempt ?? 0) < MAX_RETRY_ATTEMPTS && (
                        <button
                          type="button"
                          className="retry-answer"
                          onClick={() =>
                            void send(item.retry_question, {
                              retry: true,
                              retryOf: item.analytics_id,
                              retryAttempt: (item.retry_attempt ?? 0) + 1,
                              silentUser: true,
                            })
                          }
                        >
        ...
        {busy && (
          <div className="ai-row">
            ...
            <div>
              <div className="bubble">
                Consultando documentos locais e gerando resumo...
              </div>
              <small>busca semântica + MCP + RAG em andamento</small>
            </div>
          </div>
        )}
      ...
      <div className="suggestions">
        ...
        <button
          disabled={busy}
          onClick={() => setMessage("Execute uma análise baseada nos arquivos")}
        >
          Análise
        </button>
      </div>
      ...
      <div className="composer">
        <button
          ...
          disabled={busy}
          aria-label="Anexar arquivo de texto ou imagem"
          title="Anexar arquivo de texto ou imagem"
        >
          📎
        </button>
        <textarea
