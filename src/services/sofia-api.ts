...
export async function loadAdminPipeline(
  ...
  const [pipelineResponse, traceResponse, insightResponse, embeddingsResponse] = await Promise.all([
...
export async function loadAdminReadiness(
  ...
  const response = await authFetch(`/api/admin/readiness?module_id=${encodeURIComponent(moduleId)}`)
